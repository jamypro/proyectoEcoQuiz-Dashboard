    ...
    (snap) => {
      ...
      snap.forEach((docSnap) => {
        ...
        tblUsuarios.innerHTML += `<tr>
                    ...
                    <td><div class='actions'><button class='btn-danger' onclick='deleteRecord("registros", "${
                      docSnap.id
      ...
      snap.forEach((docSnap) => {
        ...
        if (d.fechaFinalizacion && d.fechaFinalizacion.seconds) {
          const dt = new Date(
            d.fechaFinalizacion.seconds * 1000 +
              Math.floor((d.fechaFinalizacion.nanoseconds || 0) / 1000000)
          );
          fecha = dt.toLocaleString();
        }
        ...
        tblResultados.innerHTML += `
            <tr>
              ...
              <td><div class='actions'><button class='btn-danger' onclick='deleteRecord("resultados", "${
                docSnap.id
              }")'>Eliminar</button></div></td>
            </tr>
          `;
...
const deleteRecord = async (collectionName, docId) => {
  if (
    !confirm(
      `¿Estás seguro de que quieres eliminar este registro de ${collectionName}?`
    )
  )
    return;
  try {
...
const deleteAllRecords = async (collectionName) => {
  if (
    !confirm(
      `¿Estás seguro de que quieres eliminar TODOS los registros de ${collectionName}? Esta acción no se puede deshacer.`
    )
  )
    return;
  try {
